...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="container-shell py-8 md:py-14"><Link href="/shop" className="inline-flex items-center gap-2 text-sm font-semibold text-muted hover:text-orange"><ArrowLeft size={16} /> Back to shop</Link><ProductDetailClient product={result.product} reviews={result.reviews} />{related.length > 0 && <section className="mt-20"><p className="text-xs font-bold uppercase tracking-[.18em] text-orange">You may also like</p><h2 className="mt-2 font-display text-3xl font-bold text-navy">More from the counter</h2><div className="mt-7 grid grid-cols-2 gap-x-3 gap-y-9 sm:grid-cols-4">{related.map((product) => <ProductCard key={product.id} product={product} />)}</div></section>}</div>;
}
